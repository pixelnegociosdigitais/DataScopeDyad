...
const DeepseekChatWindow: React.FC<DeepseekChatWindowProps> = ({ currentUser, onBack }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const aiMessage: DeepseekMessage = {
        role: 'model',
        content: response,
        timestamp: new Date(),
      };
        ...
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="text-center max-w-md">
            ...
            <p className="text-gray-600 mb-4">
              Para usar o chat com DeepSeek, é necessário configurar a chave da API no arquivo .env.local
            </p>
            <p className="text-sm text-gray-500">
              Adicione: VITE_DEEPSEEK_API_KEY=sua_chave_aqui
            </p>
          </div>
        </div>
      </div>
