...
export const useSurveyData = ({ currentUser: hookCurrentUser, currentCompany: hookCurrentCompany }: UseSurveyDataProps): UseSurveyDataReturn => {
    ...
    const fetchSurveys = useCallback(async (companyId: string | undefined, user: User | null, company: Company | null): Promise<Survey[]> => {
        ...
        // A string de seleção agora sempre incluirá as perguntas
        let selectString = `
            id,
            title,
            company_id,
        ...
        if ((user?.role === UserRole.USER || user?.role === UserRole.ADMIN) && companyId) {
            console.log('useSurveyData: fetchSurveys - Filtrando por company_id para usuário regular ou administrador:', companyId);
            query = query.eq('company_id', companyId);
        } else if (user?.role !== UserRole.DEVELOPER && !companyId) {
            // Se não for desenvolvedor e não houver company_id (e, portanto, não há empresa para filtrar), retorna vazio.
            console.log('useSurveyData: fetchSurveys - Sem company_id e não é desenvolvedor, retornando array vazio.');
            setSurveys([]);
        ...
        if (error) {
            ...
            logActivity('ERROR', `Erro ao buscar pesquisas para a empresa ${company?.id}: ${error.message}`, 'SURVEYS', user?.id, user?.email, company?.id);
            setSurveys([]);
            setLoadingSurveys(false);
            return [];
        } else if (data) {
            ...
            const fetchedSurveys: Survey[] = (data as any[]).map((s) => {
                // O mapeamento agora é consistente para todos os papéis, pois 'questions' sempre será buscado.
                return {
                    id: s.id,
                    title: s.title,
